...
{
	if (prop == PROTECTIONZONE && isPZ()) return true;
	...
	for (const Item* item : items) {
		if (item->hasProperty(prop)) return true;
	}
...
{
	if (!item) return wxNOT_FOUND;

	int index = 0;
	if (ground) {
		if (ground == item) return index;
		index++;
	}
...
{
	if (!items.empty() && !items.back()->isMetaItem()) return items.back();
	if (ground && !ground->isMetaItem()) {
		return ground;
	}
	return nullptr;
...
{
	if (minimapColor != INVALID_MINIMAP_COLOR) return minimapColor;
...
void Tile::borderize(BaseMap* parent) { GroundBrush::doBorders(parent, this); }
...
void Tile::wallize(BaseMap* parent) { WallBrush::doWalls(parent, this); }
...
void Tile::tableize(BaseMap* parent) { TableBrush::doTables(parent, this); }

void Tile::carpetize(BaseMap* parent) { CarpetBrush::doCarpets(parent, this); }
...
void Tile::setHouse(House* house) { house_id = (house ? house->id : 0); }
