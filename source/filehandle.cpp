...
NodeFileReadHandle::NodeFileReadHandle() :
    last_was_start(false), cache(nullptr), cache_size(32768), cache_length(0), local_read_index(0), root_node(nullptr)
{
	////
}
...
MemoryNodeFileReadHandle::MemoryNodeFileReadHandle(const uint8_t* data, size_t size) { assign(data, size); }
...
MemoryNodeFileReadHandle::~MemoryNodeFileReadHandle() { freeNode(root_node); }

void MemoryNodeFileReadHandle::close() { assign(nullptr, 0); }

bool MemoryNodeFileReadHandle::renewCache() { return false; }
...
DiskNodeFileReadHandle::DiskNodeFileReadHandle(const std::string& name,
    ...
    file_size(0)
{
	...
	if (!file || ferror(file)) {
		...
		if (ver[0] != 0 || ver[1] != 0 || ver[2] != 0 || ver[3] != 0) {
			bool accepted = false;
			for (std::vector<std::string>::const_iterator id_iter = acceptable_identifiers.begin();
			     id_iter != acceptable_identifiers.end(); ++id_iter) {
...
DiskNodeFileReadHandle::~DiskNodeFileReadHandle() { close(); }
...
BinaryNode::BinaryNode(NodeFileReadHandle* file, BinaryNode* parent) :
    read_offset(0), file(file), parent(parent), child(nullptr)
{
	////
}

BinaryNode::~BinaryNode() { file->freeNode(child); }
...
BinaryNode* BinaryNode::advance()
{
	...
	if (file->error_code != FILE_NO_ERROR) return nullptr;
...
DiskNodeFileWriteHandle::~DiskNodeFileWriteHandle() { close(); }
...
MemoryNodeFileWriteHandle::~MemoryNodeFileWriteHandle() { close(); }
...
uint8_t* MemoryNodeFileWriteHandle::getMemory() { return cache; }

size_t MemoryNodeFileWriteHandle::getSize() { return local_write_index; }
...
NodeFileWriteHandle::NodeFileWriteHandle() : cache(nullptr), cache_size(0x7FFF), local_write_index(0)
{
	////
}

NodeFileWriteHandle::~NodeFileWriteHandle() { free(cache); }
