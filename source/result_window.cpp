	...
	wxPanel(parent, wxID_ANY)
{
	...
	buttonsSizer->Add(newd wxButton(this, wxID_FILE, wxT("Export")), wxSizerFlags(0).Center());
	buttonsSizer->Add(newd wxButton(this, wxID_CLEAR, wxT("Clear")), wxSizerFlags(0).Center());
...
void SearchResultWindow::OnClickExport(wxCommandEvent& WXUNUSED(event))
{
	...
	if(dialog.ShowModal() == wxID_OK) {
		...
		if(file.IsOpened()) {
			g_gui.CreateLoadBar(wxT("Exporting search result..."));

			file.Write(wxT("Generated by Remere's Map Editor " + __RME_VERSION__));
			file.Write(wxT("\n=============================================\n\n"));
			wxArrayString lines = result_list->GetStrings();
			size_t count = lines.Count();
			for(size_t i = 0; i < count; ++i) {
				file.Write(lines[i] + wxT("\n"));
