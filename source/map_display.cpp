...
void MapCanvas::OnPaint(wxPaintEvent& event)
{
	...
	if (g_gui.IsRenderingEnabled()) {
		...
		if (screenshot_buffer) drawer->TakeScreenshot(screenshot_buffer);
...
{
	if (drawer) drawer->ShowPositionIndicator(position);
}
...
MapWindow* MapCanvas::GetMapWindow() const
{
	...
	if (window) return static_cast<MapWindow*>(window);
	return nullptr;
}
...
Position MapCanvas::GetCursorPosition() const { return Position(last_cursor_map_x, last_cursor_map_y, floor); }
...
void MapCanvas::OnMouseMove(wxMouseEvent& event)
{
	if (screendragging) {
		GetMapWindow()->ScrollRelative(
		    int(g_settings.getFloat(Config::SCROLL_SPEED) * zoom * (event.GetX() - cursor_x)),
		    int(g_settings.getFloat(Config::SCROLL_SPEED) * zoom * (event.GetY() - cursor_y)));
		Refresh();
	}
...
void MapCanvas::OnMouseLeftRelease(wxMouseEvent& event) { OnMouseActionRelease(event); }

void MapCanvas::OnMouseLeftClick(wxMouseEvent& event) { OnMouseActionClick(event); }
...
void MapCanvas::OnMouseActionClick(wxMouseEvent& event)
{
	...
	if (event.ControlDown() && event.AltDown()) {
		...
		if (tile && tile->size() > 0) {
			...
			if (item && item->getRAWBrush()) g_gui.SelectBrush(item->getRAWBrush(), TILESET_RAW);
		}
				...
				if (event.AltDown() && g_gui.GetBrushSize() == 0) {
					...
					for (int y = start_map_y - 1; y <= end_map_y + 1; ++y) {
						for (int x = start_map_x - 1; x <= end_map_x + 1; ++x) {
							if ((x <= start_map_x + 1 || x >= end_map_x - 1) ||
	...
	if (g_gui.IsSelectionMode()) {
		if (dragging && (move_x != 0 || move_y != 0 || move_z != 0)) {
			...
			if (boundbox_selection) {
				if (mouse_map_x == last_click_map_x && mouse_map_y == last_click_map_y && event.ControlDown()) {
					...
					if (width == 0) {
						threads.push_back(newd SelectionThread(editor, Position(start_x, start_y, start_z),
						                                       Position(start_x, end_y, end_z)));
					} else {
						for (int i = 0; i < threadcount; ++i) {
							...
							threads.push_back(
							    newd SelectionThread(editor, Position(start_x + cleared, start_y, start_z),
							                         Position(start_x + cleared + chunksize, end_y, end_z)));
							cleared += chunksize;
							remainder -= chunksize;
						}
					}
		...
		if (dragging_draw) {
			if (brush->isSpawn()) {
				...
				int width = std::min(g_settings.getInteger(Config::MAX_SPAWN_RADIUS),
				                     ((end_map_x - start_map_x) / 2 + (end_map_y - start_map_y) / 2) / 2);
					...
					if (g_gui.GetBrushShape() == BRUSHSHAPE_SQUARE) {
						...
						for (int x = last_click_map_x - 1; x <= mouse_map_x + 1; x++) {
							for (int y = last_click_map_y - 1; y <= mouse_map_y + 1; y++) {
								if ((x <= last_click_map_x || x >= mouse_map_x) ||
								    (y <= last_click_map_y || y >= mouse_map_y)) {
									tilestoborder.push_back(Position(x, y, floor));
								}
								if ((x >= last_click_map_x && x <= mouse_map_x) &&
								    (y >= last_click_map_y && y <= mouse_map_y)) {
						...
						int start_y, end_y;
						int width = std::max(
						    std::abs(std::max(mouse_map_y, last_click_map_y) - std::min(mouse_map_y, last_click_map_y)),
						    std::abs(std::max(mouse_map_x, last_click_map_x) -
...
void MapCanvas::OnMouseCameraClick(wxMouseEvent& event)
{
	...
	if (event.ControlDown()) {
		...
		MapWindow* window = GetMapWindow();
		window->GetViewSize(&screensize_x, &screensize_y);
		window->ScrollRelative(int(-screensize_x * (1.0 - zoom) * (std::max(cursor_x, 1) / double(screensize_x))),
...
void MapCanvas::OnMouseCameraRelease(wxMouseEvent& event)
{
	...
	if (event.ControlDown()) {
		...
		MapWindow* window = GetMapWindow();
		window->GetViewSize(&screensize_x, &screensize_y);
		window->ScrollRelative(int(zoom * (2 * cursor_x - screensize_x)), int(zoom * (2 * cursor_y - screensize_y)));
		Refresh();
	}
}
...
void MapCanvas::OnWheel(wxMouseEvent& event)
{
	if (event.ControlDown()) {
		...
		int scroll_x =
		    int(screensize_x * diff * (std::max(cursor_x, 1) / double(screensize_x))) * GetContentScaleFactor();
		int scroll_y =
		    int(screensize_y * diff * (std::max(cursor_y, 1) / double(screensize_y))) * GetContentScaleFactor();
		...
		case '9': {
			...
			if (event.ControlDown()) {
				...
				if (g_gui.IsSelectionMode()) {
					...
					int view_start_map_x = view_start_x / rme::TileSize,
					    view_start_map_y = view_start_y / rme::TileSize;
...
void MapCanvas::OnKeyUp(wxKeyEvent& event) { keyCode = WXK_NONE; }

void MapCanvas::OnCopy(wxCommandEvent& WXUNUSED(event))
{
	if (g_gui.IsSelectionMode()) editor.copybuffer.copy(editor, GetFloor());
}

void MapCanvas::OnCut(wxCommandEvent& WXUNUSED(event))
{
	if (g_gui.IsSelectionMode()) editor.copybuffer.cut(editor, GetFloor());
	g_gui.RefreshView();
}
...
void MapCanvas::OnSelectGroundBrush(wxCommandEvent& WXUNUSED(event))
{
	...
	if (bb) g_gui.SelectBrush(bb, TILESET_TERRAIN);
}
...
void MapCanvas::OnSelectDoodadBrush(wxCommandEvent& WXUNUSED(event))
{
	...
	if (item) g_gui.SelectBrush(item->getDoodadBrush(), TILESET_DOODAD);
}
...
void MapCanvas::OnSelectDoorBrush(wxCommandEvent& WXUNUSED(event))
{
	...
	if (item) g_gui.SelectBrush(item->getDoorBrush(), TILESET_TERRAIN);
}
...
void MapCanvas::OnSelectWallBrush(wxCommandEvent& WXUNUSED(event))
{
	...
	if (wb) g_gui.SelectBrush(wb, TILESET_TERRAIN);
}
...
void MapCanvas::OnSelectCreatureBrush(wxCommandEvent& WXUNUSED(event))
{
	...
	if (tile->creature) g_gui.SelectBrush(tile->creature->getBrush(), TILESET_CREATURE);
}
...
void MapPopupMenu::Update()
{
	...
	wxMenuItem* copyPositionItem =
	    Append(MAP_POPUP_MENU_COPY_POSITION, "&Copy Position", "Copy the position as a lua table");
			...
			if (topSelectedItem || topCreature || topItem) {
				...
				if (topSelectedItem && (topSelectedItem->isBrushDoor() || topSelectedItem->isRoteable() || teleport)) {
					...
					if (teleport) {
						...
						wxMenuItem* goto_menu =
						    Append(MAP_POPUP_MENU_GOTO, "&Go To Destination", "Go to the destination of this teleport");
						goto_menu->Enable(enabled);
						wxMenuItem* dest_menu = Append(MAP_POPUP_MENU_COPY_DESTINATION, "Copy &Destination",
						                               "Copy the destination of this teleport");
				...
				if (topCreature)
					Append(MAP_POPUP_MENU_SELECT_CREATURE_BRUSH, "Select Creature",
					       "Uses the current creature as a creature brush");

				if (topSpawn) Append(MAP_POPUP_MENU_SELECT_SPAWN_BRUSH, "Select Spawn", "Select the spawn brush");

				Append(MAP_POPUP_MENU_SELECT_RAW_BRUSH, "Select RAW", "Uses the top item as a RAW brush");

				if (hasWall)
					Append(MAP_POPUP_MENU_SELECT_WALL_BRUSH, "Select Wallbrush",
					       "Uses the current item as a wallbrush");

				if (hasCarpet)
					Append(MAP_POPUP_MENU_SELECT_CARPET_BRUSH, "Select Carpetbrush",
					       "Uses the current item as a carpetbrush");

				if (hasTable)
					Append(MAP_POPUP_MENU_SELECT_TABLE_BRUSH, "Select Tablebrush",
					       "Uses the current item as a tablebrush");

				if (topSelectedItem && topSelectedItem->getDoodadBrush() &&
				    topSelectedItem->getDoodadBrush()->visibleInPalette())
				...
				if (tile->hasGround() && tile->getGroundBrush() && tile->getGroundBrush()->visibleInPalette())
					Append(MAP_POPUP_MENU_SELECT_GROUND_BRUSH, "Select Groundbrush",
					       "Uses the current item as a groundbrush");
				...
				if (tile->hasGround() && tile->getGroundBrush() && tile->getGroundBrush()->visibleInPalette()) {
					Append(MAP_POPUP_MENU_SELECT_GROUND_BRUSH, "Select Groundbrush",
					       "Uses the current tile as a groundbrush");
				}
...
void MapCanvas::getTilesToDraw(int mouse_map_x, int mouse_map_y, int floor, PositionVector* tilestodraw,
	...
	if (fill) {
		...
		for (int y = -g_gui.GetBrushSize() - 1; y <= g_gui.GetBrushSize() + 1; y++) {
			for (int x = -g_gui.GetBrushSize() - 1; x <= g_gui.GetBrushSize() + 1; x++) {
				if (g_gui.GetBrushShape() == BRUSHSHAPE_SQUARE) {
					if (x >= -g_gui.GetBrushSize() && x <= g_gui.GetBrushSize() && y >= -g_gui.GetBrushSize() &&
					    y <= g_gui.GetBrushSize()) {
						if (tilestodraw) tilestodraw->push_back(Position(mouse_map_x + x, mouse_map_y + y, floor));
					}
					if (std::abs(x) - g_gui.GetBrushSize() < 2 && std::abs(y) - g_gui.GetBrushSize() < 2) {
						if (tilestoborder) tilestoborder->push_back(Position(mouse_map_x + x, mouse_map_y + y, floor));
					}
				} else if (g_gui.GetBrushShape() == BRUSHSHAPE_CIRCLE) {
					double distance = sqrt(double(x * x) + double(y * y));
					if (distance < g_gui.GetBrushSize() + 0.005) {
...
{
	if (map_canvas->GetZoom() <= 2.0) map_canvas->Refresh();
};
