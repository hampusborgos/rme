...
bool DrawingOptions::isOnlyColors() const noexcept { return show_as_minimap || show_only_colors; }

bool DrawingOptions::isTileIndicators() const noexcept
{
	if (isOnlyColors()) return false;
	return show_pickupables || show_moveables || show_houses || show_spawns;
}

bool DrawingOptions::isTooltips() const noexcept { return show_tooltips && !isOnlyColors(); }

bool DrawingOptions::isDrawLight() const noexcept { return show_ingame_box && show_lights; }
...
void MapDrawer::Draw()
{
	...
	DrawLiveCursors();
	DrawBrush();
	if (options.show_grid && zoom <= 10.f) DrawGrid();
	if (options.show_ingame_box) DrawIngameBox();
	if (options.isTooltips()) DrawTooltips();
}
						...
						for (int map_x = 0; map_x < 4; ++map_x) {
							for (int map_y = 0; map_y < 4; ++map_y) {
								...
								if (location && options.isDrawLight()) {
									...
									if (position.x >= box_start_map_x && position.x <= box_end_map_x &&
									    position.y >= box_start_map_y && position.y <= box_end_map_y) {
										AddLight(location);
									}
								}
							}
...
{
	if (!dragging || options.ingame || editor.getSelection().isBusy()) return;
	...
	for (Tile* tile : editor.getSelection()) {
		...
		if (move_x == 0 && move_y == 0 && move_z == 0) continue;
...
{
	if (!options.transparent_floors || floor == 0 || floor == 8) return;
	...
	for (int map_x = start_x; map_x <= end_x; map_x++) {
		for (int map_y = start_y; map_y <= end_y; map_y++) {
			...
			bool hidden = options.hide_items_when_zoomed && zoom > 10.f;
			if (!hidden && !tile->items.empty()) {
				for (const Item* item : tile->items) BlitItem(draw_x, draw_y, tile, item, false, 255, 255, 255, 96);
			}
		}
	}
...
{
	if (options.ingame || !editor.IsLive()) return;
	...
	for (LiveCursor& cursor : live.getCursorList()) {
		...
		if (cursor.pos.z < floor) {
			cursor.color = wxColor(cursor.color.Red(), cursor.color.Green(), cursor.color.Blue(),
...
void MapDrawer::DrawBrush()
{
	...
	if (dragging_draw) {
		...
		if (brush->isWall()) {
			...
			if (brush->isRaw()) glEnable(GL_TEXTURE_2D);
			...
			if (g_gui.GetBrushShape() == BRUSHSHAPE_SQUARE || brush->isSpawn() /* Spawn brush is always square */) {
				if (brush->isRaw() || brush->isOptionalBorder()) {
					...
					if (brush->isRaw()) raw_brush = brush->asRaw();
				...
				int start_x, end_x;
				int start_y, end_y;
				int width = std::max(std::abs(std::max(mouse_map_y, canvas->last_click_map_y) -
...
void MapDrawer::BlitItem(int& draw_x, int& draw_y, const Tile* tile, const Item* item, bool ephemeral, int red,
                         int green, int blue, int alpha)
{
	...
	if (type.isMetaItem()) return;
	if (!ephemeral && type.pickupable && !options.show_items) return;
	...
	if (!ephemeral && options.transparent_items && (!type.isGroundTile() || sprite->width > 1 || sprite->height > 1) &&
	    !type.isSplash() && (!type.isBorder || sprite->width > 1 || sprite->height > 1)) {
		alpha /= 2;
	}
	...
	for (int cx = 0; cx != sprite->width; cx++) {
		for (int cy = 0; cy != sprite->height; cy++) {
			for (int cf = 0; cf != sprite->layers; cf++) {
				...
				glBlitTexture(screenx - cx * rme::TileSize, screeny - cy * rme::TileSize, texnum, red, green, blue,
				              alpha);
			}
		}
	}

	if (options.show_hooks && (type.hookSouth || type.hookEast)) DrawHookIndicator(draw_x, draw_y, type);
}

void MapDrawer::BlitItem(int& draw_x, int& draw_y, const Position& pos, const Item* item, bool ephemeral, int red,
                         int green, int blue, int alpha)
{
	const ItemType& type = g_items.getItemType(item->getID());
	if (type.id == 0) return;
	...
	if (options.show_hooks && (type.hookSouth || type.hookEast) && zoom <= 3.0) DrawHookIndicator(draw_x, draw_y, type);
}
...
void MapDrawer::BlitCreature(int screenx, int screeny, const Outfit& outfit, Direction dir, int red, int green,
                             int blue, int alpha)
{
	if (outfit.lookItem != 0) {
			...
			if (GameSprite* mountSpr = g_gui.gfx.getCreatureSprite(outfit.lookMount)) {
				for (int cx = 0; cx != mountSpr->width; ++cx) {
					for (int cy = 0; cy != mountSpr->height; ++cy) {
						...
						glBlitTexture(screenx - cx * rme::TileSize, screeny - cy * rme::TileSize, texnum, red, green,
						              blue, alpha);
					}
				}
		...
		for (int pattern_y = 0; pattern_y < sprite->pattern_y; pattern_y++) {
			...
			if (pattern_y > 0 && !(outfit.lookAddon & (1 << (pattern_y - 1)))) continue;
...
void MapDrawer::BlitCreature(int screenx, int screeny, const Creature* creature, int red, int green, int blue,
                             int alpha)
{
...
void MapDrawer::WriteTooltip(const Item* item, std::ostringstream& stream)
{
	...
	if (unique == 0 && action == 0 && text.empty()) return;
...
void MapDrawer::DrawTile(TileLocation* location)
{
	...
	if (options.show_only_modified && !tile->isModified()) return;
	...
	if (show_tooltips && location->getWaypointCount() > 0) {
		...
		if (waypoint) WriteTooltip(waypoint, tooltip);
	}
	...
	if (only_colors || tile->hasGround()) {
		...
		if (only_colors) {
			...
			if (options.show_preview && zoom <= 2.0) tile->ground->animate();

			BlitItem(draw_x, draw_y, tile, tile->ground, false, r, g, b);
		}

		if (show_tooltips && position.z == floor) WriteTooltip(tile->ground, tooltip);
	}
	...
	if (!hidden && !tile->items.empty()) {
		for (Item* item : tile->items) {
			if (show_tooltips && position.z == floor) WriteTooltip(item, tooltip);

			if (options.show_preview && zoom <= 2.0) item->animate();
...
void MapDrawer::DrawBrushIndicator(int x, int y, Brush* brush, uint8_t r, uint8_t g, uint8_t b)
{
	...
	glBegin(GL_POLYGON);
	for (int i = 0; i < 8; ++i) glVertex2i(vertexes[i][0] + x, vertexes[i][1] + y);
	glEnd();
...
{
	if (z != pos_indicator.z || pos_indicator.x < start_x || pos_indicator.x > end_x || pos_indicator.y < start_y ||
...
{
	if (!options.show_tooltips || tooltips.empty()) return;
	...
	for (MapTooltip* tooltip : tooltips) {
		...
		for (const char* c = text; *c != '\0'; c++) {
			...
			if (tooltip->ellipsis && char_count > (MapTooltip::MAX_CHARS + 3)) break;
		}
		...
		glBegin(GL_POLYGON);
		for (int i = 0; i < 8; ++i) glVertex2f(vertexes[i][0], vertexes[i][1]);
		glEnd();
		...
		if (zoom <= 1.0) {
			...
			for (const char* c = text; *c != '\0'; c++) {
				...
				if (tooltip->ellipsis && char_count >= MapTooltip::MAX_CHARS) {
					...
					if (char_count >= (MapTooltip::MAX_CHARS + 2)) break;
				} else if (!iscntrl(*c)) {
...
void MapDrawer::glColor(const wxColor& color) { glColor4ub(color.Red(), color.Green(), color.Blue(), color.Alpha()); }

void MapDrawer::glColor(MapDrawer::BrushColor color)
{
	switch (color) {
		case COLOR_BRUSH:
			glColor4ub(g_settings.getInteger(Config::CURSOR_RED), g_settings.getInteger(Config::CURSOR_GREEN),
		...
		case COLOR_HOUSE_BRUSH:
			glColor4ub(g_settings.getInteger(Config::CURSOR_ALT_RED), g_settings.getInteger(Config::CURSOR_ALT_GREEN),
