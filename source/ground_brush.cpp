	...
	for (pugi::xml_node childNode = node.first_child(); childNode; childNode = childNode.next_sibling()) {
		...
		if (childName == "item") {
			...
			for (pugi::xml_node subChildNode = childNode.first_child(); subChildNode;
				...
				for (pugi::xml_node superChildNode = subChildNode.first_child(); superChildNode;
				     superChildNode = superChildNode.next_sibling()) {
					const std::string& superChildName = as_lower_str(superChildNode.name());
					if (superChildName == "conditions") {
						for (pugi::xml_node conditionChild = superChildNode.first_child(); conditionChild;
							...
							if (conditionName == "match_border") {
								...
								if (it == g_brushes.borders.end()) {
									warnings.push_back("Unknown border id in specific case match block " +
						...
						for (pugi::xml_node actionChild = superChildNode.first_child(); actionChild;
						     actionChild = actionChild.next_sibling()) {
			...
			for (std::vector<BorderBlock*>::iterator it = borders.begin(); it != borders.end(); ++it) {
				BorderBlock* bb = *it;
				if (bb->autoborder) {
		...
		if (second) {
			if (first->getZ() < second->getZ() && second->hasOuterBorder()) {
				if (first->hasInnerBorder()) {
					for (std::vector<BorderBlock*>::iterator it = first->borders.begin(); it != first->borders.end();
					     ++it) {
						BorderBlock* bb = *it;
						if (bb->outer) {
							continue;
				...
				for (std::vector<BorderBlock*>::iterator it = second->borders.begin(); it != second->borders.end();
				     ++it) {
					BorderBlock* bb = *it;
					if (!bb->outer) {
						continue;
...
{
	static const auto extractGroundBrushFromTile = [](BaseMap* map, uint32_t x, uint32_t y,
	                                                  uint32_t z) -> GroundBrush* {
								...
								for (BorderCluster& borderCluster : borderList) {
									if (borderCluster.border == borderBlock->autoborder) {
										...
										if (!borderBlock->specific_cases.empty()) {
											if (std::find(specificList.begin(), specificList.end(), borderBlock) ==
											    specificList.end()) {
			...
			*/
			...
			for (Item* item : tile->items) {
				...
				if (specificCaseBlock->match_group > 0) {
					...
					// item->getBorderAlignment(), specificCaseBlock->group_match_alignment);
