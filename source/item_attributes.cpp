...
{
	if (o.attributes) attributes = newd ItemAttributeMap(*o.attributes);
}

ItemAttributes::~ItemAttributes() { clearAllAttributes(); }

void ItemAttributes::createAttributes()
{
	if (!attributes) attributes = newd ItemAttributeMap;
}
...
const std::string* ItemAttributes::getStringAttribute(const std::string& key) const
{
	...
	ItemAttributeMap::iterator iter = attributes->find(key);
	if (iter != attributes->end()) return iter->second.getString();
	return nullptr;
}
...
const int32_t* ItemAttributes::getIntegerAttribute(const std::string& key) const
{
	...
	ItemAttributeMap::iterator iter = attributes->find(key);
	if (iter != attributes->end()) return iter->second.getInteger();
	return nullptr;
}
...
const double* ItemAttributes::getFloatAttribute(const std::string& key) const
{
	...
	ItemAttributeMap::iterator iter = attributes->find(key);
	if (iter != attributes->end()) return iter->second.getFloat();
	return nullptr;
}
...
const bool* ItemAttributes::getBooleanAttribute(const std::string& key) const
{
	...
	ItemAttributeMap::iterator iter = attributes->find(key);
	if (iter != attributes->end()) return iter->second.getBoolean();
	return nullptr;
}

bool ItemAttributes::hasStringAttribute(const std::string& key) const { return getStringAttribute(key) != nullptr; }

bool ItemAttributes::hasIntegerAttribute(const std::string& key) const { return getIntegerAttribute(key) != nullptr; }
...
ItemAttribute::ItemAttribute(const std::string& str) : type(ItemAttribute::STRING) { new (data) std::string(str); }

ItemAttribute::ItemAttribute(int32_t i) : type(ItemAttribute::INTEGER) { *reinterpret_cast<int*>(data) = i; }

ItemAttribute::ItemAttribute(double f) : type(ItemAttribute::DOUBLE) { *reinterpret_cast<double*>(data) = f; }

ItemAttribute::ItemAttribute(bool b) { *reinterpret_cast<bool*>(data) = b; }

ItemAttribute::ItemAttribute(const ItemAttribute& o) : type(ItemAttribute::NONE) { *this = o; }
...
ItemAttribute::~ItemAttribute() { clear(); }
...
{
	if (type == STRING) return reinterpret_cast<const std::string*>(&data);
	return nullptr;
}

const int32_t* ItemAttribute::getInteger() const
{
	if (type == INTEGER) return reinterpret_cast<const int32_t*>(&data);
	return nullptr;
}

const double* ItemAttribute::getFloat() const
{
	if (type == DOUBLE) return reinterpret_cast<const double*>(&data);
	return nullptr;
}

const bool* ItemAttribute::getBoolean() const
{
	if (type == BOOLEAN) return reinterpret_cast<const bool*>(&data);
	return nullptr;
}
...
bool ItemAttributes::unserializeAttributeMap(const IOMap& maphandle, BinaryNode* stream)
{
	...
	if (stream->getU16(n)) {
		...
		while (n--) {
			...
			if (!attrib.unserialize(maphandle, stream)) return false;
...
bool ItemAttribute::unserialize(const IOMap& maphandle, BinaryNode* stream)
{
	...
	switch (rtype) {
		case STRING: {
			std::string str;
			if (!stream->getLongString(str)) return false;
			set(str);
			break;
		}
		case INTEGER: {
			uint32_t u32;
			if (!stream->getU32(u32)) return false;
		...
		case DOUBLE: {
			uint64_t u64;
			if (!stream->getU64(u64)) return false;
			set(*reinterpret_cast<double*>(&u64));
			break;
		}
		case BOOLEAN: {
			uint8_t b;
			if (!stream->getU8(b)) return false;
			set(b != 0);
		}
		default:
			break;
