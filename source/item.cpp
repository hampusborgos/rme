...
Item::Item(unsigned short _type, unsigned short _count) : id(_type), subtype(1), selected(false), frame(0)
{
	if (hasSubtype()) {
		subtype = _count;
	}
...
Item* Item::deepCopy() const
{
	...
	if (copy) {
		...
		if (attributes) copy->attributes = newd ItemAttributeMap(*attributes);
	}
	return copy;
}
...
{
	if (old_item == nullptr) return nullptr;
...
void Item::setID(uint16_t new_id) { id = new_id; }

void Item::setSubtype(uint16_t _subtype) { subtype = _subtype; }
...
uint16_t Item::getSubtype() const { return hasSubtype() ? subtype : 0; }
...
bool Item::hasProperty(enum ITEMPROPERTY prop) const
{
	...
	switch (prop) {
		case BLOCKSOLID:
			if (type.unpassable) return true;
			break;

		case MOVEABLE:
			if (type.moveable && getUniqueID() == 0) return true;
			break;
			/*
			        case HASHEIGHT:
			            if(it.height != 0 )
		...
		case BLOCKPROJECTILE:
			if (type.blockMissiles) return true;
			break;

		case BLOCKPATHFIND:
			if (type.blockPathfinder) return true;
			break;

		case HOOK_SOUTH:
			if (type.hookSouth) return true;
			break;

		case HOOK_EAST:
			if (type.hookEast) return true;
			break;

		case BLOCKINGANDNOTMOVEABLE:
			if (type.unpassable && (!type.moveable || getUniqueID() != 0)) return true;
			break;
...
void Item::setUniqueID(unsigned short n) { setAttribute("uid", n); }

void Item::setActionID(unsigned short n) { setAttribute("aid", n); }

void Item::setText(const std::string& str) { setAttribute("text", str); }

void Item::setDescription(const std::string& str) { setAttribute("desc", str); }
...
bool Item::canHoldDescription() const { return g_items.getItemType(id).allowDistRead; }
...
{
	const ItemType& type = g_items.getItemType(id);
	if (type.isWall && type.brush && type.brush->isWall()) return type.brush->asWall();
	return nullptr;
}
...
void Item::animate()
{
	...
	if (!sprite || !sprite->animator) return;
