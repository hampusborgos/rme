...
void PropertiesWindow::saveAttributesPanel()
{
	...
	{
		std::string label = std::string(attributesGrid->GetCellValue(rowIndex, 0).mb_str());
...
void PropertiesWindow::OnGridValueChanged(wxGridEvent& event)
{
	if (event.GetCol() == 1)
	{
		...
		// It did, cool cool
		std::string label = std::string(attributesGrid->GetCellValue(event.GetRow(), 0).mb_string());
		ItemAttribute attr;
		if (newType == "String")
			attr.set("");
