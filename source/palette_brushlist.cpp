...
BrushPalettePanel::BrushPalettePanel(wxWindow* parent, const TilesetContainer& tilesets, TilesetCategoryType category,
                                     wxWindowID id) :
    PalettePanel(parent, id), palette_type(category), choicebook(nullptr), size_panel(nullptr)
{
...
PaletteType BrushPalettePanel::GetType() const { return palette_type; }
...
BrushPanel::BrushPanel(wxWindow* parent) :
    wxPanel(parent, wxID_ANY), tileset(nullptr), brushbox(nullptr), loaded(false), list_type(BRUSHLIST_LISTBOX)
{
...
void BrushPanel::OnClickListBoxRow(wxCommandEvent& event)
{
	...
	if (w) g_gui.ActivatePalette(static_cast<PaletteWindow*>(w));
...
void BrushIconBox::EnsureVisible(size_t n) { EnsureVisible(brush_buttons[n]); }
...
BrushListBox::BrushListBox(wxWindow* parent, const TilesetCategory* tileset) :
    wxVListBox(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLB_SINGLE), BrushBoxInterface(tileset)
{
...
wxCoord BrushListBox::OnMeasureItem(size_t n) const { return 32; }
