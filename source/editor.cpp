    ...
    replace_brush(nullptr)
{
	...
	if (defaultVersion == CLIENT_VERSION_NONE) defaultVersion = ClientVersion::getLatestVersion()->getID();
	...
	if (!ok) throw std::runtime_error("Couldn't load client version");
    ...
    replace_brush(nullptr)
{
	...
	if(ver < CLIENT_VERSION_760) {
	    long b = g_gui.PopupDialog("Error", "Unsupported Client Version (pre 7.6), do you want to try to load the map
	anyways?", wxYES | wxNO); if(b == wxID_NO) { valid_state = false; return;
	    }
	}
	*/
	...
	if (success) {
		...
		if(success && ver.client == CLIENT_VERSION_854_BAD) {
		    int ok = g_gui.PopupDialog("Incorrect OTB", "This map has been saved with an incorrect OTB version, do you
		want to convert it to the new OTB version?\n\nIf you are not sure, click Yes.", wxYES | wxNO);
...
Action* Editor::createAction(ActionIdentifier type) { return actionQueue->createAction(type); }

Action* Editor::createAction(BatchAction* parent) { return actionQueue->createAction(parent); }

BatchAction* Editor::createBatch(ActionIdentifier type) { return actionQueue->createBatch(type); }

void Editor::addBatch(BatchAction* action, int stacking_delay) { actionQueue->addBatch(action, stacking_delay); }

void Editor::addAction(Action* action, int stacking_delay) { actionQueue->addAction(action, stacking_delay); }

bool Editor::canUndo() const { return actionQueue->canUndo(); }

bool Editor::canRedo() const { return actionQueue->canRedo(); }

void Editor::undo(int indexes)
{
	if (indexes <= 0 || !actionQueue->canUndo()) return;

	while (indexes > 0) {
		if (!actionQueue->undo()) break;
		indexes--;
	}
	g_gui.UpdateActions();
	g_gui.RefreshView();
...
{
	if (indexes <= 0 || !actionQueue->canRedo()) return;

	while (indexes > 0) {
		if (!actionQueue->redo()) break;
		indexes--;
	}
	g_gui.UpdateActions();
	g_gui.RefreshView();
...
void Editor::resetActionsTimer() { actionQueue->resetTimer(); }
...
void Editor::clearChanges() { map.clearChanges(); }
...
void Editor::saveMap(FileName filename, bool showdialog)
{
	...
	// Save the map
	{
		...
		f << backup_otbm << std::endl << backup_house << std::endl << backup_spawn << std::endl;
	}

	{
		...
		if (showdialog) g_gui.CreateLoadBar("Saving OTBM map...");
	...
	if (!save_as && g_settings.getInteger(Config::ALWAYS_MAKE_BACKUP)) {
		...
		if (!backup_otbm.empty()) {
			...
			std::rename(backup_otbm.c_str(),
			            std::string(otbm_filename + "." + date.str() + (save_otgz ? ".otgz" : ".otbm")).c_str());
		}
...
bool Editor::importMap(FileName filename, int import_x_offset, int import_y_offset, int import_z_offset,
                       ImportType house_import_type, ImportType spawn_import_type)
{
			...
			switch (house_import_type) {
				...
				case IMPORT_SMART_MERGE: {
					if (current_town) {
						...
						if (current_town->getName() == imported_town->getName() &&
						    current_town->getID() == imported_town->getID()) {
							// Just add to map
		...
		for (HouseMap::iterator hit = imported_map.houses.begin(); hit != imported_map.houses.end();) {
			...
			switch (house_import_type) {
				...
				case IMPORT_SMART_MERGE: {
					if (current_house) {
						// Compare and insert/merge depending on parameters
						if (current_house->name == imported_house->name &&
						    current_house->townid == imported_house->townid) {
							// Just add to map
		...
		if (!resizemap && (new_pos.x > map.getWidth() || new_pos.y > map.getHeight())) {
			if (resize_asked) {
				...
				int ret = g_gui.PopupDialog(
				    "Collision",
				    "The imported tiles are outside the current map scope. Do you want to resize the map? (Else additional tiles will be removed)",
				    wxYES | wxNO);
	...
	for (std::map<Position, Spawn*>::iterator spawn_iter = spawn_map.begin(); spawn_iter != spawn_map.end();
	     ++spawn_iter) {
	...
	g_gui.PopupDialog(
	    "Success", "Map imported successfully, " + i2ws(discarded_tiles) + " tiles were discarded as invalid.", wxOK);
...
void Editor::moveSelection(const Position& offset)
{
	...
	bool create_borders = g_settings.getInteger(Config::USE_AUTOMAGIC) && g_settings.getInteger(Config::BORDERIZE_DRAG);
...
void Editor::destroySelection()
{
	if (selection.size() == 0) {
		...
		wxString ss;
		ss << "Deleted " << tile_count << " tile" << (tile_count > 1 ? "s" : "") << " (" << item_count << " item"
		   << (item_count > 1 ? "s" : "") << ")";
...
void Editor::drawInternal(Position offset, bool alt, bool dodraw)
{
	...
	if (brush->isDoodad()) {
		...
		if (!house_exit_brush->canDraw(&map, offset)) return;
		...
		if (!waypoint_brush->canDraw(&map, offset)) return;

		Waypoint* waypoint = map.waypoints.getWaypoint(waypoint_brush->getWaypoint());
		if (!waypoint || waypoint->pos == offset) return;
	...
	if (brush->isGround() || brush->isEraser()) {
		...
		for (PositionVector::const_iterator it = tilestodraw.begin(); it != tilestodraw.end(); ++it) {
			...
			if (tile) {
				Tile* new_tile = tile->deepCopy(map);
				// Wall cleaning is exempt from automagic
				if (brush->isWall()) new_tile->cleanWalls(brush->asWall());
				if (dodraw)
...
bool Editor::IsLiveClient() const { return live_client != nullptr; }

bool Editor::IsLiveServer() const { return live_server != nullptr; }

bool Editor::IsLive() const { return IsLiveClient() || IsLiveServer(); }

bool Editor::IsLocal() const { return !IsLive(); }

LiveClient* Editor::GetLiveClient() const { return live_client; }

LiveServer* Editor::GetLiveServer() const { return live_server; }

LiveSocket& Editor::GetLive() const
{
	if (live_server) return *live_server;
	return *live_client;
}
