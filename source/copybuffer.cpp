...
CopyBuffer::CopyBuffer() : tiles(newd BaseMap()) { ; }

size_t CopyBuffer::GetTileCount() { return tiles ? (size_t)tiles->size() : 0; }
...
void CopyBuffer::copy(Editor& editor, int floor)
{
	...
	for (Tile* tile : editor.getSelection()) {
		...
		if (copied_tile->getX() < copyPos.x) copyPos.x = copied_tile->getX();

		if (copied_tile->getY() < copyPos.y) copyPos.y = copied_tile->getY();
	}

	std::ostringstream ss;
	ss << "Copied " << tile_count << " tile" << (tile_count > 1 ? "s" : "") << " (" << item_count << " item"
	   << (item_count > 1 ? "s" : "") << ")";
...
void CopyBuffer::cut(Editor& editor, int floor)
{
	...
	std::stringstream ss;
	ss << "Cut out " << tile_count << " tile" << (tile_count > 1 ? "s" : "") << " (" << item_count << " item"
	   << (item_count > 1 ? "s" : "") << ")";
	g_gui.SetStatusText(wxstr(ss.str()));
}
...
bool CopyBuffer::canPaste() const { return tiles && tiles->size() != 0; }
