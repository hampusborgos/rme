...
bool Materials::loadExtensions(FileName directoryName, wxString& error, wxArrayString& warnings)
{
	...
	do {
		...
		MaterialsExtension* materialExtension =
		    newd MaterialsExtension(extensionName, extensionAuthor, extensionDescription);
		...
		if ((attribute = extensionNode.attribute("client"))) {
			...
			std::sort(materialExtension->version_list.begin(), materialExtension->version_list.end(),
			          VersionComparisonPredicate);

			auto duplicate =
			    std::unique(materialExtension->version_list.begin(), materialExtension->version_list.end());
...
bool Materials::unserializeMaterials(const FileName& filename, pugi::xml_node node, wxString& error,
...
bool Materials::isInTileset(Item* item, std::string tilesetName) const
{
	...
	return type.id != 0 && (isInTileset(type.brush, tilesetName) || isInTileset(type.doodad_brush, tilesetName) ||
...
bool Materials::isInTileset(Brush* brush, std::string tilesetName) const
{
	...
	if (tilesetiter == tilesets.end()) return false;
