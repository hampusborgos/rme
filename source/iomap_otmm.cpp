...
{
	uint16_t _id;
	if (!stream->getU16(_id)) return nullptr;
...
bool Item::readItemAttribute_OTMM(const IOMap& maphandle, OTMM_ItemAttribute attr, BinaryNode* stream)
{
	switch (attr) {
		case OTMM_ATTR_SUBTYPE: {
			uint16_t subtype;
			if (!stream->getU16(subtype)) return false;
		...
		case OTMM_ATTR_UNIQUE_ID: {
			uint16_t uid;
			if (!stream->getU16(uid)) return false;

			setUniqueID(uid);
		} break;
		case OTMM_ATTR_TEXT: {
			std::string text;
			if (!stream->getString(text)) return false;
...
void Item::serializeItemCompact_OTMM(const IOMap& maphandle, NodeFileWriteHandle& stream) const { stream.addU16(id); }
...
bool IOMapOTMM::loadMap(Map& map, NodeFileReadHandle& f, const FileName& identifier, bool showdialog)
{
	...
	if (!root->getU32(u32) || u32 > (unsigned long)g_items.MajorVersion) { // OTB major version
		if (queryUser(
		        "Map error",
		        "The loaded map appears to be a items.otb format that deviates from the items.otb loaded by the editor. Do you still want to attempt to load the map?")) {
			...
			switch (node_type) {
				...
				case OTMM_TILE_DATA: {
					...
					if (tileNode) do {
							...
							uint8_t z_offset;
							if (!tileNode->getU16(x_offset) || !tileNode->getU16(y_offset) ||
									...
									if (item_type == OTMM_ITEM) {
										...
										if (item) {
											if (item->unserializeItemNode_OTMM(*this, itemNode) == false) {
												warning("Couldn't unserialize item attributes at %d:%d:%d", pos.x,
												        pos.y, pos.z);
											}
											tile->addItem(item);
										}
				...
				case OTMM_SPAWN_DATA: {
					...
					if (spawnNode) do {
							...
							uint8_t spawn_z;
							uint32_t radius;
							if (!spawnNode->getU16(spawn_x) || !spawnNode->getU16(spawn_y) ||
							...
							if (spawn_tile && spawn_tile->spawn) {
								warning("Duplicate spawn on position %d:%d:%d\n", spawn_tile->getX(),
								        spawn_tile->getY(), spawn_tile->getZ());
								continue;
							}
							...
							if (creatureNode) do {
									...
									uint8_t creature_z;
									if (!creatureNode->getU16(creature_x) || !creatureNode->getU16(creature_y) ||
									...
									// Check radius
									if (uint32_t(abs(creaturepos.x - spawnpos.x)) > radius ||
									    uint32_t(abs(creaturepos.y - spawnpos.y)) > radius) {
									...
									if (!creature_tile) {
										warning("Discarding creature \"%s\" at %d:%d:%d due to invalid position",
										        name.c_str(), creaturepos.x, creaturepos.y, creaturepos.z);
										break;
									}
									if (creature_tile->creature) {
										warning("Duplicate creature \"%s\" at %d:%d:%d, discarding", name.c_str(),
									...
									if (creature_tile->spawn_count == 0) {
										// No spawn, create a newd one (this happends if the radius of the spawn has
										// been decreased due to g_settings)
					...
					f.addNode(save_tile->isHouseTile() ? OTMM_HOUSETILE : OTMM_TILE);
					{
						...
						if (save_tile->ground) {
							...
							if (ground->isMetaItem()) {
								...
								bool found = false;
								for (ItemVector::const_iterator it = save_tile->items.begin();
								     it != save_tile->items.end(); ++it) {
						...
						for (ItemVector::const_iterator it = save_tile->items.begin(); it != save_tile->items.end();
						     ++it) {
							if (!(*it)->isMetaItem()) {
