...
Houses::Houses(Map& map) : map(map), max_house_id(0)
{
	////
}
...
Houses::~Houses()
{
	for (auto it = houses.begin(); it != houses.end(); ++it) delete it->second;
	houses.clear();
}

uint32_t Houses::getEmptyID() { return ++max_house_id; }
...
void Houses::addHouse(House* new_house)
{
	...
	new_house->map = &map;
	if (new_house->id > max_house_id) max_house_id = new_house->id;
...
void Houses::removeHouse(House* house_to_remove)
{
	...
	if (it != houses.end()) houses.erase(it);
...
const House* Houses::getHouse(uint32_t houseid) const
{
	...
	if (it != houses.end()) return it->second;

	return nullptr;
}

House::House(Map& map) : id(0), rent(0), townid(0), guildhall(false), map(&map), exit(0, 0, 0)
{
	////
}
...
void House::clean()
{
	for (PositionList::const_iterator pos_iter = tiles.begin(); pos_iter != tiles.end(); ++pos_iter) {
		...
		if (tile) tile->setHouse(nullptr);
	}

	Tile* tile = map->getTile(exit);
	if (tile) tile->removeHouseExit(this);
}
...
uint8_t House::getEmptyDoorID() const
{
	...
	for (PositionList::const_iterator tile_iter = tiles.begin(); tile_iter != tiles.end(); ++tile_iter) {
		if (const Tile* tile = map->getTile(*tile_iter)) {
			for (ItemVector::const_iterator item_iter = tile->items.begin(); item_iter != tile->items.end();
			     ++item_iter) {
				if (Door* door = dynamic_cast<Door*>(*item_iter)) taken.insert(door->getDoorID());
			}
		}
	}
...
{
	// This might fail when the user decides to put an exit at 0,0,0, let's just hope noone does (Noone SHOULD, so there
	// is no problem? Hm?)
	if (pos == exit || !pos.isValid()) return;

	if (exit.isValid()) {
		Tile* oldexit = targetmap->getTile(exit);
		if (oldexit) oldexit->removeHouseExit(this);
	}
...
void House::setExit(const Position& pos) { setExit(map, pos); }
