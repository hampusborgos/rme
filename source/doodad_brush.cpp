...
DoodadBrush::~DoodadBrush()
{
	for (std::vector<AlternativeBlock*>::iterator alt_iter = alternatives.begin(); alt_iter != alternatives.end();
	     ++alt_iter) {
		delete *alt_iter;
	}
}

DoodadBrush::AlternativeBlock::AlternativeBlock() : composite_chance(0), single_chance(0)
{
	////
}
...
DoodadBrush::AlternativeBlock::~AlternativeBlock()
{
	for (std::vector<CompositeBlock>::iterator composite_iter = composite_items.begin();
	     composite_iter != composite_items.end(); ++composite_iter) {
		...
		for (CompositeTileList::iterator compt_iter = tv.begin(); compt_iter != tv.end(); ++compt_iter) {
			...
			for (ItemVector::iterator iiter = items.begin(); iiter != items.end(); ++iiter) delete *iiter;
		}
	}

	for (std::vector<SingleBlock>::iterator single_iter = single_items.begin(); single_iter != single_items.end();
	     ++single_iter) {
...
bool DoodadBrush::loadAlternative(pugi::xml_node node, wxArrayString& warnings, AlternativeBlock* which)
{
	...
	for (pugi::xml_node childNode = node.first_child(); childNode; childNode = childNode.next_sibling()) {
		...
		if (childName == "item") {
			...
			for (pugi::xml_node compositeNode = childNode.first_child(); compositeNode;
			     compositeNode = compositeNode.next_sibling()) {
				...
				ItemVector items;
				for (pugi::xml_node itemNode = compositeNode.first_child(); itemNode;
...
{
	for (std::vector<SingleBlock>::const_iterator single_iter = single_items.begin(); single_iter != single_items.end();
	     ++single_iter) {
	...
	for (std::vector<CompositeBlock>::const_iterator composite_iter = composite_items.begin();
		...
		for (CompositeTileList::const_iterator comp_iter = ctl.begin(); comp_iter != ctl.end(); ++comp_iter) {
			...
			for (ItemVector::const_iterator item_iter = items.begin(), item_end = items.end(); item_iter != item_end;
			     ++item_iter) {
...
bool DoodadBrush::ownsItem(Item* item) const
{
	...
	for (std::vector<AlternativeBlock*>::const_iterator alt_iter = alternatives.begin(); alt_iter != alternatives.end();
	     ++alt_iter) {
...
void DoodadBrush::draw(BaseMap* map, Tile* tile, void* parameter)
{
	...
	for (std::vector<SingleBlock>::const_iterator block_iter = ab_ptr->single_items.begin();
	     block_iter != ab_ptr->single_items.end(); ++block_iter) {
...
const CompositeTileList& DoodadBrush::getComposite(int variation) const
{
	...
	for (std::vector<CompositeBlock>::const_iterator block_iter = ab_ptr->composite_items.begin();
	     block_iter != ab_ptr->composite_items.end(); ++block_iter) {
...
{
	if (hasCompositeObjects(variation)) return false;
	if (hasSingleObjects(variation)) return false;
