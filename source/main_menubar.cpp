...
#define MAKE_SET_ACTION(id, kind, setting_, handler) actions[#id] = new MenuBar::Action(#id, id, kind, wxCommandEventFunction(&MainMenuBar::handler)); actions[#id].setting = setting_
	...
	for(std::map<std::string, MenuBar::Action*>::iterator ai = actions.begin();
		ai != actions.end();
		++ai)
	{
...
{
	std::map<MenuBar::ActionID, std::list<wxMenuItem*> >::iterator fi =
		items.find(id);
	if(fi == items.end())
		return;
...
{
	std::map<MenuBar::ActionID, std::list<wxMenuItem*> >::const_iterator fi =
		items.find(id);
	if(fi == items.end())
		return false;
...
#define ANALYZE_ITEM(_item) {\
	item_count += 1; \
	if((_item)->isGroundTile() == false && (_item)->isBorder() == false) { \
		is_detailed = true; \
...
#undef ANALYZE_ITEM
	...
	for(HouseMap::const_iterator hit = houses.begin();
			hit != houses.end();
			++hit)
	{
