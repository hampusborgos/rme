...
ItemDatabase::~ItemDatabase() { clear(); }
...
bool ItemDatabase::loadFromOtbVer1(BinaryNode* itemNode, wxString& error, wxArrayString& warnings)
{
	for (; itemNode != nullptr; itemNode = itemNode->advance()) {
		...
		if (group == ITEM_GROUP_DEPRECATED) continue;
		...
		if (itemNode->getU32(flags)) {
			...
			item->floorChange = item->floorChangeDown || item->floorChangeNorth || item->floorChangeEast ||
			                    item->floorChangeSouth || item->floorChangeWest;
		...
		while (itemNode->getU8(attribute)) {
			...
			switch (attribute) {
				case ITEM_ATTR_SERVERID: {
					...
					if (!itemNode->getU16(item->id)) warnings.push_back("Invalid item type property (2)");

					if (maxItemId < item->id) maxItemId = item->id;
					break;
				}
				...
				case ITEM_ATTR_CLIENTID: {
					...
					if (!itemNode->getU16(item->clientID)) warnings.push_back("Invalid item type property (2)");
				...
				case ITEM_ATTR_LIGHT2: {
					if (datalen != sizeof(lightBlock2)) {
						warnings.push_back("items.otb: Unexpected data length of item light (2) block (Should be " +
						                   i2ws(sizeof(lightBlock2)) + " bytes)");
						break;
					}
				...
				case ITEM_ATTR_TOPORDER: {
					if (datalen != sizeof(uint8_t)) {
						warnings.push_back(
						    "items.otb: Unexpected data length of item toporder block (Should be 1 byte)");
						break;
					}

					uint8_t value = 0;
					if (!itemNode->getU8(value)) warnings.push_back("Invalid item type property (5)");
				...
				case ITEM_ATTR_NAME: {
					if (datalen >= 128) {
						warnings.push_back(
						    "items.otb: Unexpected data length of item name block (Should be 128 bytes)");
						break;
					}
				...
				case ITEM_ATTR_DESCR: {
					if (datalen >= 128) {
						warnings.push_back(
						    "items.otb: Unexpected data length of item descr block (Should be 128 bytes)");
						break;
					}
				...
				case ITEM_ATTR_MAXITEMS: {
					if (datalen != sizeof(unsigned short)) {
						warnings.push_back(
						    "items.otb: Unexpected data length of item volume block (Should be 2 bytes)");
						break;
					}

					if (!itemNode->getU16(item->volume)) warnings.push_back("Invalid item type property (8)");
					break;
				}
				...
				case ITEM_ATTR_WEIGHT: {
					if (datalen != sizeof(double)) {
						warnings.push_back(
						    "items.otb: Unexpected data length of item weight block (Should be 8 bytes)");
						break;
					}
				...
				case ITEM_ATTR_ROTATETO: {
					if (datalen != sizeof(unsigned short)) {
						warnings.push_back(
						    "items.otb: Unexpected data length of item rotateTo block (Should be 2 bytes)");
						break;
					}
...
bool ItemDatabase::loadItemFromGameXml(pugi::xml_node itemNode, uint16_t id)
{
	...
	for (pugi::xml_node itemAttributesNode = itemNode.first_child(); itemAttributesNode;
	     itemAttributesNode = itemAttributesNode.next_sibling()) {
...
{
	if (id == 0 || id > maxItemId) return dummy;
...
{
	if (id == 0 || id > maxItemId) return nullptr;
	return items[id];
}

bool ItemDatabase::isValidID(uint16_t id) const
{
	if (id == 0 || id > maxItemId) return false;
