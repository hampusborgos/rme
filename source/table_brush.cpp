...
TableBrush::TableBrush() : look_id(0)
{
	////
}
...
bool TableBrush::canDraw(BaseMap* map, const Position& position) const { return true; }
...
void TableBrush::draw(BaseMap* map, Tile* tile, void* parameter)
{
	...
	for (std::vector<TableType>::const_iterator table_iter = tn.items.begin(); table_iter != tn.items.end();
	     ++table_iter) {
