...
LiveSocket::LiveSocket() :
    ...
    mapReader(nullptr, 0),
    mapWriter(),
    mapVersion(MapVersion(MAP_OTBM_4, CLIENT_VERSION_NONE)),
    log(nullptr),
    name("User"),
    password("")
{
...
wxString LiveSocket::getName() const { return name; }
...
wxString LiveSocket::getPassword() const { return password; }
...
wxString LiveSocket::getLastError() const { return lastError; }

void LiveSocket::setLastError(const wxString& error) { lastError = error; }

std::string LiveSocket::getHostName() const { return "?"; }
...
void LiveSocket::receiveNode(NetworkMessage& message, Editor& editor, Action* action, int32_t ndx, int32_t ndy,
                             bool underground)
{
	QTreeNode* node = editor.getMap().getLeaf(ndx * 4, ndy * 4);
	if (!node) {
		log->Message("Warning: Received update for unknown tile (" + std::to_string(ndx * 4) + "/" +
		             std::to_string(ndy * 4) + "/" + (underground ? "true" : "false") + ")");
		return;
	}
...
void LiveSocket::receiveFloor(NetworkMessage& message, Editor& editor, Action* action, int32_t ndx, int32_t ndy,
                              int32_t z, QTreeNode* node, Floor* floor)
{
...
void LiveSocket::sendFloor(NetworkMessage& message, Floor* floor)
{
	...
	std::string stream(reinterpret_cast<char*>(mapWriter.getMemory()), mapWriter.getSize());
